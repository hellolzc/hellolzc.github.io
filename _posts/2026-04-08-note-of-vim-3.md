...
插件airline和vim-fern需要一些特殊符号，所以建议安装针对性修改的字体，建议一步到位nerdfont。可以在Google搜索 "字体名+Nerd Font Mono" 查找对应的字体。比如Consolas这个字体，有好心人已经提供了打包好的版本（https://github.com/wclr/my-nerd-fonts.git），这里还有一个升级版（https://github.com/hellolzc/my-nerd-fonts）。我个人喜欢 Hack Font，这个字体和很多其他开源字体都可以从这里下载https://www.nerdfonts.com/font-downloads。
...
filetype plugin indent on            " 开启文件类型检测，并加载插件和缩进相关设置
...
function! s:toggle_number_info()
    set list!
    set number!
    let &foldcolumn = (&foldcolumn == 0 ? 1 : 0)
endfunction
...
" 使用 L/H 切换到 下一个/上一个 buffer （丢掉 H/L 跳屏幕顶底，这个不常用）
noremap L :bn<CR>
noremap H :bp<CR>
...
nnoremap <Leader>uh :call <SID>toggle_number_info()<CR>
" 切换自动换行（与 F4 功能相同，在F4被占用时使用）
nnoremap <Leader>uw :call <SID>toggle_wrap()<CR>
" <leader>ul: 切换普通行号
...
同时，这次的配置加入了快捷键提示。WhichKey插件（vim-which-key）是一个极佳的快捷键提示工具，能动态显示以`<leader>`开头的映射，极大降低记忆负担。按下`<leader>`再稍等一会儿就可以看到提示。
...
在这次的配置中，切换buffer支持使用`<space><buffer编号>`，此外使用 `L` 和 `H` 在切换到 下一个/上一个 buffer，比默认方便了很多。
...
这次的改动参考了大量lazynvim内容，并且得到了Gemini和Claude的强力支持，在此表示感谢~


## 邪修用法

nvim其实可以使用vimrc格式的配置文件！只要 `cp .vimrc ./.config/nvim/init.vim`!
我下面做了简单修改，就可以在nvim中使用这份配置文件了，背景色有点不一致，其他都正常工作。
考虑到nvim可以去github直接下载appimage，这个方法适合连vim-nox都安装不上的人！
nodejs、tree-sitter、Telescope全部都可以放弃，红红火火恍恍惚惚~

```diff
$ diff .vimrc ./.config/nvim/init.vim
7a8
...
此外，我还发现nvim中tabline支持点击切换buffer了，意外之喜！

neovim中甚至还支持系统剪贴板交互，使用OSC 52剪贴板，下面这段配置可以强制启动该功能：
```
" 设置 y 直接同步到系统剪贴板
...
vim.g.clipboard = {
  ...
  paste = {
    -- Windows Terminal 不支持读取，设置为空防止卡死
    ['+'] = function() return {} end,
    ['*'] = function() return {} end,
  },
}
